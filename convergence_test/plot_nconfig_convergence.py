...
# nconfigs = [5,10,15,20,25,30,35,40,45]
nconfigs = [5,15,25,35,45]
colors = ['blue','green','red','orange','yellow']
std = 3
xspace = np.linspace(0,500,5000)
npeaks = 3

peaks = np.zeros((npeaks,len(nconfigs),2))

for i,nconfig in enumerate(nconfigs):
    avg_intens = np.zeros(len(xspace))
    for config in range(nconfig):

        filename = f'nconfig_convergence_data/nconfigs_{nconfig}_config_{config}.npz'
        npzfile = np.load(filename)
        intens,freqs = npzfile['intensities'],npzfile['freqs']
        avg_intens = avg_intens + get_spectra_curve(xspace,intens,freqs,std)[1]
    avg_intens /= nconfig
    axs[0].plot(xspace,avg_intens,label=f'$N_{{c}}$={nconfig}',color=colors[i])
...
axs[0].arrow(peaks[0,-1,0]-30,peaks[0,-1,1]+10,28,-7,width=0.5)
axs[0].text(peaks[0,-1,0]-90,peaks[0,-1,1]+11,'Peak 1')

axs[0].arrow(peaks[1,-1,0]-30,peaks[1,-1,1]+5,25,-3,width=0.5)
axs[0].text(peaks[1,-1,0]-90,peaks[1,-1,1]+6,'Peak 2')

axs[0].arrow(peaks[2,-1,0]-30,peaks[2,-1,1]+2,21,0,width=0.5)
axs[0].text(peaks[2,-1,0]-90,peaks[2,-1,1]+1,'Peak 3')
...
axs[0].text(-0.08,0.95,'(a)',fontsize=fontsize+4,fontweight='extra bold',transform=axs[0].transAxes)

#fig,axes = plt.subplots(2,1,sharex=True)
axs[1].plot(nconfigs,peaks[0,:,0],marker='o',c='b')
axs[1].axhline(peaks[0,-1,0],c='k',linestyle='--')
axs[1].text(min(nconfigs),peaks[0,-1,0]+9,'Peak 1',c='b')
axs[1].plot(nconfigs,peaks[1,:,0],marker='o',c='g')
axs[1].axhline(peaks[1,-1,0],c='k',linestyle='--')
axs[1].text(min(nconfigs),peaks[1,-1,0]-25,'Peak 2',c='g')
axs[1].plot(nconfigs,peaks[2,:,0],marker='o',c='r')
axs[1].axhline(peaks[2,-1,0],c='k',linestyle='--')
axs[1].text(min(nconfigs),peaks[2,-1,0]-22,'Peak 3',c='r')
axs[1].set_ylabel(r'Raman shift (cm$^{-1}$)')
axs[1].set_xlabel(f'$N_{{c}}$')
...
axs[2].plot(nconfigs,peaks[0,:,1],marker='o',c='b')
axs[2].axhline(peaks[0,-1,1],c='k',linestyle='--')
axs[2].text(min(nconfigs),peaks[0,-1,1]+4,'Peak 1',c='b')
axs[2].plot(nconfigs,peaks[1,:,1],marker='o',c='g')
axs[2].axhline(peaks[1,-1,1],c='k',linestyle='--')
axs[2].text(min(nconfigs),peaks[1,-1,1]-11,'Peak 2',c='g')
axs[2].plot(nconfigs,peaks[2,:,1],marker='o',c='r')
axs[2].axhline(peaks[2,-1,1],c='k',linestyle='--')
axs[2].text(min(nconfigs),peaks[2,-1,1]-7,'Peak 3',c='r')
axs[2].set_ylabel('Intensity (a.u.)'                )
axs[2].set_xlabel(f'$N_{{c}}$')
axs[2].yaxis.set_minor_locator(MultipleLocator(5))
axs[2].text(-0.08,0.95,'(c)',fontsize=fontsize+4,fontweight='extra bold',transform=axs[2].transAxes)

plt.tight_layout()
plt.savefig('freqs_intens_vs_nconfigs',dpi=300,facecolor='white',transparent=False,
